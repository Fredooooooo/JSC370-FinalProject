...
knitr::opts_knit$set(root.dir = 'E:\Github\JSC370-FinalProject')
```
...
## Methods
The dataset we used is the ATP World Tour tennis data. “This dataset contains tennis data from the ATP World Tour website. The data contains ATP tournaments, rankings and player’s overview. The latest available data is for 2017.” (ATP World Tour, 2018) The dataset can be downloaded from the url: 'https://datahub.io/sports-data/atp-world-tour-tennis-data/datapackage.json' or website 'https://datahub.io/sports-data/atp-world-tour-tennis-data#r', we only require the 9th and 10th csv file from this json file. We provided two methods to read in the dataset. The first one gives the url path provided by the json file to read.csv(), which will takes a long time to download from the website. The second one requires the downloaded csv and zip file stored in the same path as the rmd file, and then apply read_csv() directly on the path 'rankings_1973-2017_csv.csv' and 'player_overviews_unindexed_csv.csv'.
...
Besides gam model, we also ##########Adding description about other ML models
...
We also explored #######age and countries interactive plots
